...
# =========================================================
# Section 5: Dropbox sync (mirrors steps/26_dropbox_sync.sh)
# =========================================================

print_section "Dropbox sync (rclone bisync + systemd)"

DROPBOX_PAIRS=(
  # rclone.conf intentionally not tracked: it holds the OAuth token,
  # which is per-machine and generated by `rclone config`.
  "files/systemd/rclone-dropbox.service" "$HOME/.config/systemd/user/rclone-dropbox.service"
  "files/systemd/rclone-dropbox.timer"   "$HOME/.config/systemd/user/rclone-dropbox.timer"
)

for ((i = 0; i < ${#DROPBOX_PAIRS[@]}; i += 2)); do
  check_file "$SCRIPT_DIR/${DROPBOX_PAIRS[i]}" "${DROPBOX_PAIRS[i + 1]}"
done
